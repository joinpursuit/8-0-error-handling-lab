...
function getCartTotal(cart) {
  if (cart.length === 0) throw "The cart is empty.";
...
function filterProductsByPriceRange(products, min, max) {
  ...
  if (products.length === 0) throw "The cart is empty.";
  if (!Number.isInteger(min) || !Number.isInteger(max)) throw "The input is not a valid number.";
  if (max <= 0 || min < 0 || min > max) throw "Must input the correct number.";

  for (let product of products) {
    if (!product.priceInCents) throw "Must input the price of the product.";
    if (product.priceInCents >= min && product.priceInCents <= max) result.push(product);
  }
  return result;
}
