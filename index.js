...
function filterProductsByPriceRange(products, min, max) {
  if(products.length === 0) throw "is empty"
  if(typeof min !== 'number'|| typeof max !== 'number') throw 'not a number'
  if(min > max) throw 'min cant be larger'
  if(min < 0 || max < 0) throw 'cant be negative'
  if(max === 0) throw 'cant be zero'
  const result = [];
  for (let product of products) {
    if(!product.priceInCents) throw 'missing price'
