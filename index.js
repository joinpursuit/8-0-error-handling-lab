...
function filterProductsByPriceRange(products, min, max) {
  //Switch statement to handle the edge cases
  switch (true){
    case !products.length:
      throw 'Error: Products array is empty.';
      break;
    case typeof min !== 'number' || typeof max !== 'number' :
      throw 'Error: min or max is not a number';
      break;
    case max === 0:
      throw 'Error: Max is 0';
      break;
    case min > max:
      throw 'Error: min is > max';
      break;
    case min < 0 || max < 0:
      throw 'Error: min or max is < 0'
      break;
  }
  const result = [];
  for (let product of products) {
    if (!product.priceInCents){
      throw "Error: Product doesn't have a priceInCents key"
    }
