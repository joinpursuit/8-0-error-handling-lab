...
function filterProductsByPriceRange(products, min, max) {
  if(products.length === 0){throw "products array empty"};
  if(typeof(min)!== "number"){throw "min is NaN"};
  if(typeof(max)!== "number"){throw "max is NaN"};
  if(max === 0){throw "max is 0"};
  if(max < 0){throw "max is less than 0"};
  if(min < 0){throw "min is less than 0"}
  if(min > max){throw "min is greater than max"}
  const result = [];
  for (let product of products) {
  if (product.priceInCents === undefined){
    throw "product.priceInCents undefined"
  }
