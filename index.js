...
function filterProductsByPriceRange(products, min, max) {
  if (products.length === 0) {
    ...
    throw `Min or Max value is not a number`
  } else if (min > max) {
    throw `Max value is bigger than min value`
  } else if (min < 0 || max <= 0) {
    throw `Min or Max value needs to greater than 0`
  } else if (max === 0) {
    throw `Max needs to greater than 0`
  }
