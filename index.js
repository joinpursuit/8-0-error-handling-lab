...
function filterProductsByPriceRange(products, min, max) {
  if (products.length === 0 || typeof(min) !== "number" || typeof(max) !== "number" || max === 0 || min > max || min < 0 || max < 0){
    throw `there is an error somewhere on your code :):`
  }
  const result = [];
  for (let product of products) {
    if(!product.priceInCents){
      throw `You need a price`
    }
