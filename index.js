...
function filterProductsByPriceRange(products, min, max) {
  ...
  if (typeof min !== 'number' || typeof max !== 'number') {
    throw 'Max and min must be numbers'
  }
  if (min > max) {
    throw 'min cannot be less than max'
  }
  if (min < 0 || max < 0) {
    throw 'min must be greater than or equal to 0, max must be greater than 0'
  }
  if (max === 0) {
  ...
  for (let product of products) {
    if (product.priceInCents === undefined) throw 'No price in cents data'
