...
function getCartTotal(cart) {
  ...
  if (cart.length === 0) {
    throw "Insufficient items in cart"
  }
...
function filterProductsByPriceRange(products, min, max) {
  ...
  if (typeof min !== 'number' || typeof max !== 'number') {
    throw "min/max must be a number!"
  }

  if (max === 0) {
    throw "max can not be zero"
  }
 
  if (min > max) {
    throw "max can not be greater than min"
  }

  if (min < 0 || max < 0) {
    throw "min/max must be a positive number"
  }
  const result = [];
  for (let product of products) {
    if (!product.priceInCents) { // checks if each product has a price
      throw "Product does not have a 'priceInCents' key"
    }
