...
function filterProductsByPriceRange(products, min, max) {
  ...
  if (products.length < 1 || typeof min !== 'number' || typeof max !== 'number' || min > max || min < 0 || max < 0 || max === 0) {
    throw 0
  } else {
    return result;
  }
...
function getTotalOfAllProductsByPriceRange(products, min, max) {
  try {
    const getAllPriceRange = filterProductsByPriceRange(products, min, max);
    const total = getCartTotal(getAllPriceRange);
    return total
  } catch (error) {
    return 0
  }
