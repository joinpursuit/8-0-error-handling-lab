...
/*
  ...
  - should throw an error if any of the products is missing the `priceInCents` key
*/
...
function filterProductsByPriceRange(products, min, max) {
  ...
  for (let product of products) {
    if(!product.priceInCents){
      throw 'there is no `priceInCents` key'
    }
...
function getTotalOfAllProductsByPriceRange(products, min, max) {
  try { const filterProducts = filterProductsByPriceRange(products, min, max);
    const total = getCartTotal(filterProducts)
