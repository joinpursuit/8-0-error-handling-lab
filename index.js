...
function filterProductsByPriceRange(products, min, max) {
  ...
  else if (typeof min !== 'number' || typeof max !== 'number') throw 'Error'
  else if (max === 0) throw 'Error'
  else if (min > max) throw 'Error'
  else if (min < 0 || max <0) throw 'Error'

  const result = [];
  for (let product of products) {
    if (!product.priceInCents) throw 'Error'
